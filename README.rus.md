...
Тестировалось и отлаживалось на Ubuntu 24.04 и 26.04
...
podman run --detach --name awg-rootless --publish 3400:51820/udp -v amneziawg-cfg:/etc/amnezia/amneziawg/ \
 --cap-add net_admin --sysctl net.ipv4.conf.all.src_valid_mark=1 --sysctl net.ipv4.ip_forward=1 --sysctl net.ipv4.ip_unprivileged_port_start=53 \
...
- Дополнительные параметры AWG (опционально, по умолчанию пустые - не добавляются в конфиг, если не определены):
  - `ASC_S3` (целое, 0-65535): размер мусорных данных для cookie reply пакетов
  - `ASC_S4` (целое, 0-65535): размер мусорных данных для transport пакетов
  - `ASC_I1` .. `ASC_I5` (строка): спецификации цепочек обфускации в формате тегов, передаются в демон AWG как есть
- Формат H1-H4: принимаются как целочисленные значения (старый формат), так и новый формат диапазона (`N-N`, например `1000-2000`)
...
podman run --detach --name awg-rootless --publish 4450:51820/udp \
 ...
 --env WG_INTERNAL_SUBNET="192.168.254.0/24" --env WG_CUSTOM_MTU=1320 docker.io/andrico21/amneziawg-rootless
```
Логи контейнера:
```bash
